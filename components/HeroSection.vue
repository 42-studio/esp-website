...
let imageBlur = ref(false);
const images = [
  new URL(
    "~/assets/img/product/cyber-defense-matrix-dark.png",
    import.meta.url
  ),
  ...
  new URL(
    "~/assets/img/product/product-profile-contracts-dark.png",
    import.meta.url
  ),
];
...
setTimeout(() => {
  setInterval(() => {
    imageBlur.value = true;
  }, 10000);
}, 9500);
setTimeout(() => {
  setInterval(() => {
    imageBlur.value = false;
  }, 10000);
}, 10500);
</script>
  ...
  <section class="relative" ref="observables">
      ...
      <div class="w-6/12 mx-auto md:mr-0 animated delay-3s zoomIn">
        ...
        >
          <path
            ...
            style="
              fill: none;
              stroke: url(#esp-icon-gradient);
              stroke-width: 4px;
            "
          />
          <path
            d="M15.446,519.442L420.735,924.732L420.735,797.758C420.735,797.758 168.199,545.222 137.987,515.009C136.544,513.567 136.381,511.329 137.498,509.707L496.127,150.98L496.127,24.005C496.127,24.005 76.517,443.615 15.446,504.687C11.371,508.761 11.371,515.368 15.446,519.442Z"
            style="fill: url(#esp-icon-gradient); stroke: none; stroke-width: 0"
          />
          <defs>
            <linearGradient
              id="esp-icon-gradient"
              x1="0"
              y1="0"
              x2="1"
              y2="0"
              gradientUnits="userSpaceOnUse"
              gradientTransform="matrix(941.625,1537,-793.807,1537,79.5613,124.359)"
            >
              <stop offset="0" style="stop-color: #5093ea" />
              <stop offset="1" style="stop-color: #e11b71" />
            </linearGradient>
          </defs>
        </svg>
      </div>
    ...
    <div class="flex flex-col lg:flex-row">
      <div class="flex flex-col w-full lg:w-2/6 lg:mt-[-40%]">
        <div class="shrink">
          <div
            class="inline-block mb-8 rounded-full p-2 px-4 text-sm ring-1 ring-slate-300/10 text-slate-200 animated delay-1s fadeInDown"
          >
            It's Time To
          </div>
          ...
          <div class="max-w-2xl animated delay-2s fadeInLeft">
            ...
            <p class="mt-6 text-lg leading-8 text-slate-300">
              ...
              /><span class="animated flash infinite -ml-1">|</span>
              <br />
              with security threats?
            </p>

            <p class="mt-6 text-lg leading-8 text-slate-300">
              How would that transform enterprise security strategy and
              investment?
            </p>
        ...
        <div class="grow">
          <div class="mt-8 ml-16 w-6/12 intersection slow pathing-height">
            <elements-path-bottom-left
              class="intersection slower pathing-width"
            />
          </div>
        </div>
      </div>
      ...
      <div class="relative">
        <div class="animated fadeInRight delay-4s">
          <div
            class="inline-block rounded ring ring-gray-400 ring-opacity-5 overflow-clip max-h-[65vh] mx-auto my-10 lg:my-[5vh] w-full lg:w-[75vw]"
            style="max-width: 1900px"
          >
            <img
              :src="images[imageIndex]"
              class="hero-preview transition-all duration-500"
              :class="{ 'blur-lg scale-95 opacity-10': imageBlur }"
              alt="ESProfiler Capability Views"
            />
          </div>
        </div>
      </div>
